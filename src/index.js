...
*/
function listRender(choices, pointer, icons) {
  var output = '';
...
*/
function getDirectoryContent(basePath, includeHidden, includeFiles) {
  return fs
    ...
    .filter(function (file) {
      try {
        ...
        var isFile = stats.isFile() && includeFiles;
        if (includeHidden) {
...
function Prompt() {
  ...
  try {
    ...
    this.opt.canSelectFile = this.opt.displayFiles &&
      (this.opt.options.canSelectFile === undefined ? true : this.opt.options.canSelectFile);
  ...
  try {
    this.opt.icons = this.opt.options.icons;
    if (this.opt.icons !== false) {
      this.opt.icons = Object.assign({}, defaultIcons, this.opt.icons)
    }
  } catch (e) {
...
Prompt.prototype.render = function (selectedPath) {
  ...
  if (this.status === 'answered') {
    ...
    message += chalk.dim('\n');
    var choicesStr = listRender(this.opt.choices, this.selected, this.opt.icons);
...
Prompt.prototype.handleSubmit = function (e) {
  ...
  var done = obx.filter(function (choiceValue) {
    var choice = self.opt.choices.getChoice(self.selected);
    return choiceValue === CURRENT || (self.opt.canSelectFile && choice.isFile);
  }).take(1);

  var back = obx.filter(function (choiceValue) {
    return choiceValue === BACK;
  }).takeUntil(done);

  var drill = obx.filter(function (choiceValue) {
    return choiceValue !== BACK && choiceValue !== CURRENT;
  }).takeUntil(done);
...
/**
* when user selects "." or a file
*/
Prompt.prototype.onSubmit = function ( /* choiceValue */ ) {
  var choice = this.opt.choices.getChoice(this.selected);
  var selectedPath = path.resolve(this.opt.basePath, this.currentPath, choice.value);
...
Prompt.prototype.createChoices = function (basePath) {
  ...
  var directoryContent = getDirectoryContent(basePath, this.opt.displayHidden, this.opt.displayFiles);
